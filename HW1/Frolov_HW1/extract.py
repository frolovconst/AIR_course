...
def read_pdfs_other(path, the_file):
	files_contents = []
	i=-1
	i+=1
	if the_file[-3:]!='pdf':
...
def extract_other_pdfs(file_number):
	dataset_directory = 'data/other/'
	files = listdir(dataset_directory)
	file_number = file_number % len(files)
	test_docs = read_pdfs_other(dataset_directory, files[file_number-1])
	test_out = open("test_other.txt","w")
	
	for document in test_docs:
		test_out.write(document + "\n")
